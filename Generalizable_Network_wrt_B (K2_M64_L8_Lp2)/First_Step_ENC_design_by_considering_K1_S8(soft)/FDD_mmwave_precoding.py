...
with tf.name_scope("DL_training_phase"):
    ...
    lay['aneal'] = tf.constant(1.0) #Annealing parameter (Not Used Here!)
    ############### User's operations
...
with tf.name_scope("UEs_operations"):
    ...
    for kk in range(K):
        ...
        InfoBits_soft[kk] = Dense(units=S, activation='tanh')(dense_layer3)
        
        if kk == 0:
            DNN_input_BS = InfoBits_soft[kk]
        else:
            DNN_input_BS = tf.concat([DNN_input_BS,InfoBits_soft[kk]],axis=1)     
#######################################################################################
...
with tf.Session() as sess:
    ...
    #Reading the parameters of the designed ENC        
    ...
    sio.savemat('Data_K1_weight_bias.mat',dict(A1=A1,A2=A2,A3=A3,A4=A4,\
                                            ...
                                            Xp_r=Xp_r,Xp_i=Xp_i))
    loss_test_Final,InfoBits_soft_ev = sess.run([loss,InfoBits_soft], feed_dict=feed_dict_test_Final)
    rate_MRT, rate_ZF = generate_MRT_ZF_performance(h_act_test_Final,noise_std_dl,P)
    print("Mean rate of the test set:")
    print(-loss_test_Final)   
    sio.savemat('Data_K{}M{}Lp{}_resultS{}L{}.mat'.format(K,M,Lp,S,L),dict(loss_test_Final=loss_test_Final,\
                                            rate_MRT=rate_MRT, rate_ZF=rate_ZF,S=S,L=L))   
    softBits = np.reshape(InfoBits_soft_ev[0],[-1,1])
    print("Distribution of the tanh layer:")
    plt.hist(softBits, bins = 100)
    sio.savemat('softBits_S{}.mat'.format(S),dict(S=S,softBits=softBits))  
