...
for cnt = 1:length(Lpvec)
    ...
    str = sprintf('Data_K2M64B30L64_resultLp%d',Lpvec(cnt));    
    load(str);
    f(cnt) = -loss_test_Final_B30;
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % DNN-based w/ joint design
    str = sprintf('Data_K2M64B30L64_resultLp%d_rangeTrained',Lpvec(cnt));    
    load(str);
...
hold on;
plot(Lpvec,f_MRT_Q,'-*','color',color4,'linewidth',2,'markersize',12);
hold on;
plot(Lpvec,f_MRT_CS,'-.','color',color3,'linewidth',3,'markersize',8);
hold on;
plot(Lpvec,f_MRT_CSQ,'--+','color',color3,'linewidth',4,'markersize',12);
hold on;
plot(Lpvec,f_ZF,'-','color',color1,'linewidth',2,'markersize',6);
hold on
plot(Lpvec,f_ZF_Q,'-o','color',color1,'linewidth',2,'markersize',8);
hold on;
plot(Lpvec,f_ZF_CS,'--','color',color5,'linewidth',2,'markersize',8);
hold on;
plot(Lpvec,f_ZF_CSQ,'--<','color',color5,'linewidth',2,'markersize',6);
